...
//
#define TAG_INT  106
#define TAG_DOUBLE  107
#define TAG_ARR_DOUBLE  108
#define TAG_ARR_INT  109
	...
	public:
		...
		MPI_Status getStatus()const{return status;}
		MPI_Request getRequest()const{return request;}
		...
		void waitAll(int count,MPI_Request *r){
			MPI_Status *s=new MPI_Status[count];
			MPI_Waitall(count,r,s);
			delete [] s;
		}
		MPI_Request asend(int buf,int dest,int tag=TAG2){
#ifdef USE_MPI
			MPI_Isend(&buf,1,MPI_INT,dest,tag,MPI_COMM_WORLD,&request);
#endif
			return request;
		}
		MPI_Request arecv(int &buf,int src,int tag=TAG2){
#ifdef USE_MPI
			MPI_Irecv(&buf,1,MPI_INT,src,tag,MPI_COMM_WORLD,&request);
#endif
			return request;
		}
		void test(
